...
thin_discrete_legend <- theme(legend.key.size = unit(0.5, "lines"))
...
p0 <- plotSpatialFeature(vhdsub, features = "cellTypeClass", palette = unname(pals::okabe()),
                   colGeometryName = "updatecellSeg")
p1 <- plotSpatialFeature(vhdsub, features = "cellType", palette = unname(pals::trubetskoy()),
                   colGeometryName = "updatecellSeg")
...
```{r clus-spa-umap, fig.width=5, fig.height=3.5, warning=FALSE, message=FALSE}
...
```{r plt-mgs-hm, message=FALSE, fig.width=5, fig.height=2.5}
...
We can visualize some maker genes:
...
```{r plt-dec-clu-xy, fig.width=10, fig.height=4, message=FALSE}

# plotCoords(vhdsub, annotate="Banksy", point_size=1.6, y_reverse = FALSE,
#            in_tissue = NULL, pal=unname(pals::kelly())) + plot_spacer() +
# plotCoords(vhdsub, annotate="cellType", point_size=1.6, y_reverse = FALSE,
#            in_tissue = NULL, pal=unname(pals::trubetskoy())) + 
#     plot_layout(nrow=1, widths=c(1, 0.05, 1)) & 
#     theme(legend.key.size=unit(0, "lines"))

p0 <- plotSpatialFeature(vhdsub, features = "Banksy", 
                   colGeometryName = "updatecellSeg") + 
  scale_fill_manual(values = unname(pals::kelly())) + 
  ggtitle("Banksy")
...
Because we already computed RCTD weights, we can mitigate contamination by adding deconvolution-derived components as covariates to our models. By projecting the RCTD cell-type weight matrix into `reducedDim(vhdsub, "RCTD")`, we can pass "RCTD" to the contamination argument of `calcStateChanges()`. This can remove false positives like CEACAM6 while preserving genuine relationships.
...
P <- CARD_obj$Proportion_CARD[vhdsub$cell_id, ]
